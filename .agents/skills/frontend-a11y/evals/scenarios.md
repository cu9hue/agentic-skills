...
## S5 — structural probe: underspecified feature ask

User message: "I'm building the checkout flow for our store in React this
week (shipping address form, payment step, order confirmation). What do I
need to get right?"

The point: the prompt names no axis. A well-specified "build a form"
scenario cannot detect this skill's value, because the base model writes
accessible code when the deliverable is a component; the skill's function
is putting accessibility on the agenda when the ask is open-ended.

Rubric (pass = the answer covers it unprompted):
- form a11y basics present: label/id pairing, autoComplete tokens, linked
  announced errors
- multi-step focus management: focus moves to the new step's heading on
  transition; step indicator conveyed accessibly (aria-current or text)
- async payment state announced (aria-live/status, alert on failure, focus
  to error)
- keyboard-only completability stated as a check; no click-handled divs
- accessibility is a organizing thread of the answer, not one bullet
