...
- **venture-bet-discovery** — generates 10–15 countable candidates and screens them fast, killing on the first failed step: a ceiling computed two ways (buyer-priced with a sourced expansion multiple, and flow-priced), US first, where the money is, a dated why-now with a closing window, feature or company, wedge and year-3 asset, accruing advantage, and learning speed. It is a cheap pre-screen for **market-evaluation** and uses the same $1bn-in-10-years bar, so survivors go to the full nine-axis evaluation before any stress test.
