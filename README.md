...
- **design-decision-tree** — surfaces the decisions nobody has made yet. Two paths: a capped triage that turns raw task text into at most six questions ranked by ambiguity removed — span (how much stays undetermined until it is answered) times uncertainty (how far apart the plausible answers are), so a consequential-but-predictable decision drops below the line as a default instead of burning a slot — each carrying a concrete default, nothing dropped silently; or a full walk that resolves every ambiguity in a spec and rewrites it. Capped triage adapted from [Matt Pocock's `grilling` skill](https://github.com/mattpocock/skills/blob/main/skills/productivity/grilling/SKILL.md).
...
- **review-panel** — pre-merge code review from ten fixed angles (scalability, production readiness, edge cases, data races, domain adequacy, pattern conformance, code quality, operations, external-API currency, architecture), fanned out to four clustered reviewer subagents on multi-file diffs and run solo on one-screen diffs. Every finding is verified against the code before reporting (confirmed vs plausible, noise dropped, duplicates merged), domain conventions become questions to the user instead of assertions, and the report is severity-ranked with clean angles named. The ten angles are the author's standing review prompt, kept verbatim.
...
- **chart-design** — charts that answer a question, tell the truth, stay readable, and only then look good. Three gates before any chart code: state the question the chart answers (or don't draw it), place it on the purpose map, then fix angle/framing/focus. Audited against Kirk's priority-ordered principles — **trustworthy → accessible → elegant**, where elegance is never bought at the cost of the other two. Bundles a CHRTS chart-family lookup and a scored audit with a deception catalog. Medium-agnostic (matplotlib, d3, Recharts, SVG, slides); pairs with **ui-design** (the surrounding page) and **dashboard-builder** (Grafana/SigNoz mechanics). Spine adapted from [Andy Kirk, *Data Visualisation: A Handbook for Data Driven Design*](https://us.sagepub.com/en-us/nam/data-visualisation/book261263) (SAGE, 2016); editorial layer cross-checked against [Ben Jones, "On Visualizing Data Well"](https://dataremixed.com/2015/05/on-visualizing-data-well/), which maps Zinsser onto visualisation.
...
### Startups
- **market-evaluation** — judges whether a market can carry a $1bn+ ARR company in under 10 years. It asks nine questions in three tests: Scale ($1bn path built bottom-up, US first, year-3 adjacencies), Truth (market dynamics, problem size, pain depth, repeatability, tailwinds), and Speed (learning cycles in 18 months). Each question gets a sourced bull case, a sourced bear case, and an integer score from -5 to +5. Each test has a gate: one axis at -3 or below fails it. No total and no average. Questions paraphrased from Entrepreneurs First's Fall26 market-selection session; revenue shapes from [Christoph Janz, "Five ways to build a $100 million business"](https://christophjanz.blogspot.com/2014/10/five-ways-to-build-100-million-business.html).
- **venture-ideation** — generates a batch of 12–15 startup candidates from eight distinct origins (edge, non-consensus thesis, expired dogma, hard idea, idea maze, overlooked market, fringe behaviour, visible spend), with quotas so no single origin, domain, product shape or founder capability dominates. It asks the founder four questions (edge, stale received wisdom, a hard idea, what the smartest people they know are doing), writes each candidate as a countable buyer plus N, and hands the unranked batch to **venture-bet-discovery**. Lenses from Alice Bentinck's "Ideas Pt I/II" (Entrepreneur First), Alex Crompton's "Hard Ideas", Chris Dixon's "The Idea Maze", Elad Gil in First Round Review, and EF's Fall26 market-selection guide.
- **venture-bet-discovery** — screens a candidate batch fast, killing on the first failed step: a ceiling of customers × revenue per customer at today's sourced price, with its revenue shape, US first, where the money is, a dated why-now with a closing window, feature or company, wedge and year-3 asset, accruing advantage, and learning speed. It is a cheap pre-screen for **market-evaluation** and uses the same $1bn-in-10-years bar, so survivors go to the full nine-axis evaluation.
